...
export function TutorAppHeader({
  activeNav = 'lessons',
  session,
  notifications = [],
  onMarkAllRead,
      ...
      <div style={{ display: 'flex', alignItems: 'center', width: '100%', gap: 16 }}>
        <div style={{ flex: 1, display: 'flex', justifyContent: 'flex-start', minWidth: 0 }}>
          ...
          >
            ...
            <span style={{ fontWeight: 800, fontSize: 17, color: palette.brandText }}>ИнфаСотка</span>
          </button>
        </div>
        ...
        <div style={{ flex: 1, display: 'flex', justifyContent: 'flex-end', alignItems: 'center', gap: 10, minWidth: 0 }}>
          ...
          <div ref={profileMenuRef} style={{ position: 'relative' }}>
            ...
            {menuOpen ? (
              ...
              >
                {session?.name ? <div style={{ padding: '6px 10px 8px', fontSize: 13, color: '#6b7f9f', fontWeight: 600 }}>{session.name}</div> : null}
                <button
                  type="button"
                  onClick={() => go('/tutor/profile')}
                  style={{ display: 'block', width: '100%', textAlign: 'left', padding: '8px 10px', border: 0, background: 'transparent', cursor: 'pointer', fontWeight: 600 }}
                >
                  Профиль
                </button>
                <button
                  ...
                  style={{ display: 'block', width: '100%', textAlign: 'left', padding: '8px 10px', border: 0, background: 'transparent', cursor: 'pointer', fontWeight: 600, marginTop: 4 }}
                >
                  Выход
                </button>
              </div>
